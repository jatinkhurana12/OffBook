...
// Forwards one chunk of a resumable upload session to YouTube. This runs
// on OUR server, not in the browser — which matters, because CORS is a
// browser-only restriction. YouTube's upload endpoint doesn't send back
// headers a browser is allowed to read, so a direct browser PUT actually
// succeeds on YouTube's side (the video lands on the channel) while the
// browser still reports it as a failed connection, since it's blocked
// from reading the confirmation. Routing chunks through here instead
// avoids the browser talking to googleapis.com at all.
async function uploadChunkToYoutube({ uploadUrl, chunk, start, end, total, mimeType }) {
  ...
  if (res.status === 308) {
    // "Resume Incomplete" — expected between chunks, not an error.
    const range = res.headers.get("range"); // e.g. "bytes=0-4194303"
    const nextOffset = range ? Number(range.split("-")[1]) + 1 : end;
    return { done: false, nextOffset };
  }

  const errBody = await res.text();
  throw new Error(`YouTube rejected this chunk (status ${res.status}): ${errBody}`);
}

// Asks YouTube how many bytes of the file it actually has — used to
// recover correctly after a chunk fails, instead of blindly re-sending
// from wherever the client thinks it left off.
async function queryUploadOffset({ uploadUrl, total }) {
  const res = await fetch(uploadUrl, {
    method: "PUT",
  ...
  if (res.status === 200 || res.status === 201) {
    return total; // somehow already fully received
  }
  const errBody = await res.text();
  throw new Error(`Couldn't check upload status (status ${res.status}): ${errBody}`);
}
