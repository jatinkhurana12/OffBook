...
// Notifies the OffBook team when someone submits the "Contact Us" form on
// the About page. Sent to CONTACT_TO_EMAIL (falls back to RESEND_FROM_EMAIL's
// address if not set) so you don't need a second env var just to get started.
async function sendContactMessageEmail({ name, email, phone, message }) {
  const toEmail = process.env.CONTACT_TO_EMAIL || FROM_EMAIL.match(/<(.+)>/)?.[1] || FROM_EMAIL;

  if (!RESEND_API_KEY) {
    console.warn(
      "[mailer] RESEND_API_KEY is not set — skipping real email send.\n" +
        `[mailer] New contact message from ${name} <${email}>${phone ? ` (${phone})` : ""}:\n[mailer] ${message}`
    );
    return { skipped: true };
  }
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      text: `New message via the OffBook Contact Us form.\n\nName: ${name}\nEmail: ${email}\nPhone: ${phone || "—"}\n\nMessage:\n${message}`,
    }),
  });
...
function buildContactEmailHtml({ name, email, phone, message }) {
  ...
  return `
    <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; color: #17181A;">
      <h2 style="margin-bottom: 8px;">New Contact Us message</h2>
      <p><strong>Name:</strong> ${name}</p>
      <p><strong>Email:</strong> ${email}</p>
      <p><strong>Phone:</strong> ${phone || "—"}</p>
      <p style="margin-top:16px;"><strong>Message:</strong><br />${safeMessage}</p>
    </div>
  `;
}

// NOTE: the old "you have a new message" email notification (via Resend)
// has been removed on purpose. New-message alerts now go out as browser
// push notifications instead — see lib/push.js and app/api/messages/[id]/route.js.
// Password reset emails below are untouched.
...
module.exports = { sendPasswordResetEmail, sendContactMessageEmail };