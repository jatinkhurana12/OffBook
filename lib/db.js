...
function ensureSchema() {
  if (!schemaReady) {
    schemaReady = pool.query(`
      ...
      -- Trailblazer posts: either an article (full text lives in "body")
      -- or a video lecture (the file itself lives on OffBook's own
      -- unlisted YouTube channel — see lib/youtube.js — with only the
      -- resulting video ID kept here).
      ...
      -- Glossary: any user can mark any lecture (video or article),
      -- problem, or internship as a Glossary entry. It's purely personal —
      -- item_type + item_id point polymorphically into lectures/problems/
      -- internships, so there's no FK; the API filters out rows whose
      -- target has since been deleted.
      CREATE TABLE IF NOT EXISTS glossary_items (
        id SERIAL PRIMARY KEY,
        user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
        item_type TEXT NOT NULL CHECK (item_type IN ('lecture', 'problem', 'internship')),
        item_id INTEGER NOT NULL,
        created_at TIMESTAMPTZ DEFAULT NOW(),
        UNIQUE(user_id, item_type, item_id)
      );

      CREATE INDEX IF NOT EXISTS idx_glossary_items_user ON glossary_items(user_id, created_at DESC);
